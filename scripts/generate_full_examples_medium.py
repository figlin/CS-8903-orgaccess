#!/usr/bin/env python3
"""
Synthetic FULL-Access Example Generator for OrgAccess RBAC Dataset (MEDIUM Difficulty)

This script generates synthetic "FULL" access examples for MEDIUM difficulty level.
MEDIUM examples have 6-7 permission fields instead of 8+ (HARD level).

Methodology:
- Follows OrgAccess paper approach (Mistral-based generation)
- Varies permission coverage (50% all active, 40% subset, 10% minimal reduce)
- Maintains medium-level complexity (6-7 permission fields)
- Ensures logical consistency with RBAC policies

Usage:
    python generate_full_examples_medium.py \\
        --input medium-00000-of-00001_permission_database.json \\
        --output data/medium_full_augmented.jsonl \\
        --num-examples 1000 \\
...
SYSTEM_PROMPT = """
You are generating realistic organizational access-control examples for the OrgAccess-style MEDIUM difficulty split.
MEDIUM examples have 6-7 permission fields (simpler than HARD which has 8+ fields).
...
Guidelines:
- Keep queries moderately complex (not as complex as HARD examples)
- The user should sound natural and goal-oriented, not robotic
- Mention 3-5 relevant constraints (departments, actions, access levels, restrictions)
- Avoid repeating permission names literally; express their intent naturally
- For rationale, write like an internal auditor explaining why the request is valid

IMPORTANT: The expected response must be "full" (complete approval).

Output ONLY valid JSON with this exact structure:
{
  "user_role": "<role name>",
  "permissions": <permissions object with 6-7 fields>,
...
def load_data(input_path: Path) -> List[Dict]:
    ...
    # Filter for examples with 6-7 permissions (medium-level, vs 8+ for hard)
    filtered = []
    for ex in examples:
        perms = ex.get('permissions', {})
        if isinstance(perms, dict) and 6 <= len(perms) <= 7:
            filtered.append(ex)

    print(f"✓ Filtered to {len(filtered):,} examples with 6-7 permission fields (medium-level)")
...
def sample_permissions(
    ...
    """
    Sample/modify permissions based on augmentation strategy for MEDIUM difficulty (6-7 fields).

    Strategies:
    - 'all_active' (50%): Keep all permissions (safer for 6-7 fields)
    - 'subset' (40%): Reduce list items but keep all fields
    - 'minimal_reduce' (10%): Drop 1 field only if starting with 7
    ...
    if strategy == 'all_active':
        ...
        # For MEDIUM: keep 2-3 items (moderate complexity)
        # For boolean permissions, sometimes set to True
        modified = {}

        for key, value in permissions.items():
            if isinstance(value, list) and len(value) > 2:
                # Keep 2-3 items from the list for medium complexity
                keep_count = random.randint(2, min(3, len(value)))
    ...
    elif strategy == 'minimal_reduce':
        # Drop 1 field only if we have 7 fields (for MEDIUM difficulty)
        # Keep at least 6 fields
        if len(permissions) < 7:
            return permissions, 'minimal_reduce_skip'
        ...
        if not droppable:
            return permissions, 'minimal_reduce_no_drop'

        # Drop exactly 1 field
        to_drop = random.sample(droppable, 1)
        modified = {k: v for k, v in permissions.items() if k not in to_drop}
        return modified, 'minimal_reduce'
