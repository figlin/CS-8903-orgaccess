#!/usr/bin/env python3
"""
QLoRA Fine-Tuning Script for OrgAccess RBAC Benchmark (AUGMENTED DATA VERSION)

This version uses the expanded, balanced dataset from training_data/addOn_training_data/
- train.parquet: ~27K examples (balanced full/partial/rejected)
- validation.parquet: ~7K examples
...
Usage:
    ...
    python scripts/train_qlora_augmented.py --config configs/llama3_1_8b_qlora.yaml --wandb
"""
...
def load_and_prepare_data(config: Dict, tokenizer):
    ...
    val_file = 'training_data/addOn_training_data/validation.parquet'

    print(f"\n{'='*60}")
    print(f"Loading AUGMENTED datasets...")
    print(f"{'='*60}\n")
